...
export default function SpeakingPage() {
  ...
  // Dịch câu hỏi AI (theo index trong convTurns) — cùng pattern với app/(app)/chat/page.tsx.
  const [translations, setTranslations] = useState<Record<number, string>>({});
  const [translationErrors, setTranslationErrors] = useState<Record<number, string>>({});
  const [visibleTranslations, setVisibleTranslations] = useState<Set<number>>(new Set());
  const [translatingIdx, setTranslatingIdx] = useState<number | null>(null);

  // Gợi ý cách trả lời câu hỏi AI vừa hỏi — chỉ hiện khi bấm nút, không tự động.
  const [answerSuggestion, setAnswerSuggestion] = useState<{ suggestion: string; explanation: string } | null>(null);
  ...
  async function streamCompanionReply(history: ChatTurn[]) {
    ...
    setAnswerSuggestion(null); // câu hỏi mới -> gợi ý cũ (cho câu hỏi trước) không còn phù hợp
    setSuggestError("");
    let finalText = "";
    try {
  ...
  async function translateTurn(i: number) {
    ...
    try {
      const res = await fetch("/api/tutor/translate-message", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: convTurns[i].text }),
      });
  ...
  function toggleTranslate(i: number) {
    ...
    if (translations[i] === undefined) translateTurn(i);
  }

  // Gợi ý cách trả lời câu hỏi AI vừa hỏi — bấm mới gọi, không tự động (giữ tính
  // chủ động luyện tập). Stateless — chỉ cần câu hỏi + trình độ, không cần lịch sử.
  async function suggestAnswerForLastQuestion() {
    const lastQuestion = [...convTurns].reverse().find((t) => t.role === "model")?.text;
    if (!lastQuestion || suggesting) return;
    setSuggesting(true);
    setSuggestError("");
    try {
      const res = await fetch("/api/tutor/suggest-answer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ question: lastQuestion, level }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Lỗi gợi ý");
      setAnswerSuggestion({ suggestion: data.suggestion, explanation: data.explanation });
    } catch (err) {
      setSuggestError(err instanceof Error ? err.message : "Lỗi gợi ý");
    } finally {
      setSuggesting(false);
    }
  }
      ...
      {mode === "conversation" && (
        <>
          ...
          >
            ...
            {convTurns.map((t, i) => {
              const isLastStreaming = convLoading && i === convTurns.length - 1;
              return (
                <div key={i} className={`flex ${t.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  >
                    ...
                    {t.role === "model" && t.text && !isLastStreaming && (
          ...
          <div className="rounded-xl border border-border bg-surface p-4">
            <div className="mb-3 flex items-center justify-between gap-2">
              <button
                onClick={suggestAnswerForLastQuestion}
                disabled={!convTurns.some((t) => t.role === "model" && t.text) || convLoading || suggesting}
                className="cursor-pointer rounded-lg border border-border px-3 py-1.5 text-xs font-medium transition-colors duration-200 hover:border-primary hover:bg-primary/10 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {suggesting ? "Đang gợi ý…" : "💡 Gợi ý cách trả lời"}
              </button>
            </div>
            {suggestError && <p className="mb-3 text-xs text-danger-text">{suggestError}</p>}
            {answerSuggestion && (
              <div className="mb-3 rounded-lg border border-primary/30 bg-primary/5 p-3 text-sm">
                <p className="font-medium text-primary-text">{answerSuggestion.suggestion}</p>
                {answerSuggestion.explanation && (
                  <p className="mt-1 text-xs text-muted">{answerSuggestion.explanation}</p>
                )}
                <button
                  onClick={() => setAnswerSuggestion(null)}
                  className="mt-1.5 cursor-pointer text-xs text-muted hover:underline"
                >
                  Đóng
                </button>
              </div>
