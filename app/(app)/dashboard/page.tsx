...
import { getWeaknessSummary } from "@/lib/weakness";
...
export default async function DashboardPage() {
  ...
  const [profile, vocab, trend, recent, attemptCount, weaknesses] = await Promise.all([
    getProfile(userId),
    getVocabStats(userId),
    getAttemptTrend(userId),
    getRecentAttempts(userId, 10),
    getAttemptCount(userId),
    getWeaknessSummary(userId),
  ]);
  const todayProgress = await getTodayProgress(userId, profile.dailyGoal);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div className="rounded-xl border border-border bg-surface p-4 text-center">
          <p className="text-3xl font-semibold text-primary-text">🪙 {profile.points}</p>
          <p className="text-xs text-muted">Điểm thưởng</p>
        </div>
      ...
      <WeaknessSection items={weaknesses} />
